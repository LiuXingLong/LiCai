...
public class ActivityOutlay extends Activity{
	 private String uid;
	 ...
	 protected void onCreate(Bundle savedInstanceState) {
	        ...
	        //Toast.makeText(ActivityOutlay.this,this.uid, Toast.LENGTH_SHORT).show();
	 ...
	 public void Saveinfo(View view){
	     ...
	     String place = ((EditText)findViewById(R.id.editText3)).getText().toString().trim(); 
	     String remark = ((EditText)findViewById(R.id.editText4)).getText().toString().trim();
	     
	     if(money.equals("")||time.equals("")||place.equals("")||remark.equals("")){
	    	 Toast.makeText(ActivityOutlay.this,"请填写完信息！", Toast.LENGTH_SHORT).show();
	     }else{
	    	 if(isValidDate(time)==false){	    		 
	    		 Toast.makeText(ActivityOutlay.this,"时间格式不正确！", Toast.LENGTH_SHORT).show();
	    	 }else{
	    		 ActionDB LicaiDB=new ActionDB(this);
			     LicaiDB.addoutlay(this.uid, money, time, this.category, place, remark);		     
			     List<Outlay> Outlay=LicaiDB.findoutlay(this.uid, money, time, this.category, place, remark);
			     if(Outlay.isEmpty()){
			    	 Toast.makeText(ActivityOutlay.this,"添加失败！", Toast.LENGTH_SHORT).show();	    	
			     }else{
			    	 Toast.makeText(ActivityOutlay.this,"添加成功！", Toast.LENGTH_SHORT).show();
